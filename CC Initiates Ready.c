...
int Decision_System(int x){
  ...
  switch (x) {
    ...
    case 3:
    // 유저가 Homebrew 설치 함수에서 '계속' 요청을 하고 최초로 Atom 설치 함수에 진입하는 IF문
    ...
    case 4:
    // 유저가 Atom 설치 함수에서 '계속' 요청을 하고 최초로 AtomPackage 설치 함수에 진입하는 IF문
...
int main(){
  ...
  switch (selection) {
    ...
    case 2:
      ...
      system("open http://cache.ppomppu.co.kr/zboard/data3/2017/0529/20170529233353_yecyhdcp.jpg");
      break;
...
int CLT_Installation(){
  ...
  printf("Detailed information will not be given right now due to time constrain and the level of difficulty. But you need it.\n\n\n");
...
int AtomPackage_Installation(int x){
  ...
  printf("Well... If there is 'only' atom, it will be just a boring program that can do nothing.\n");
  printf("So, I will install a small terminal and some complier utilities to your atom.\n");
  printf("The terminal that I will install will have same function as this terminal right now.\n");
  ...
  printf("'Script' package is a program that can 'build' your codes into actual programs.\n\n");

  if (x==1){
    system("apm install script");
  }

  printf("\n\nAlthough, 'script' package requires some other programs to build your codes depending on the language you use.\n(And there are a lot of different languages.)\n");
